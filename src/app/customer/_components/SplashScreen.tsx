...
import { X, Target, ChevronRight, Loader2 } from "lucide-react";
...
export default function SplashScreen({ onClose }: SplashScreenProps) {
  ...
  const handleSelect = async (planId: string) => {
    ...
    setLoading(true);
    try {
      await apiFetch(`/campaigns/${campaign.id}/register`, {
        method: 'POST',
        body: JSON.stringify({ selected_plan: planId.toUpperCase() })
      });
      toast.success("Successfully joined the contest!");
      onClose(); // This should return the user to home
    } catch (e: unknown) {
  ...
  if (showPlanSelection) {
    return (
      <div className="fixed inset-0 z-[120] bg-[#000814] flex flex-col h-full overflow-hidden">
        <button
          ...
          <div className="flex flex-col items-center w-full px-4 py-8 gap-6">
            {['a', 'b', 'c', 'd', 'e', 'f'].map((p) => (
              <button
                ...
                className="w-full relative rounded-3xl overflow-hidden shadow-2xl transition-all active:scale-[0.98] border border-white/10 group disabled:opacity-50"
              >
                <Image
  ...
  if (showParticipationGuide) {
    return (
      <div className="fixed inset-0 z-[100] bg-[#041226] flex flex-col h-full overflow-hidden">
        <button
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-slate-900 flex justify-center overflow-hidden h-full">
      <div className="w-full max-w-lg bg-[#041226] text-white flex flex-col font-sans relative overflow-hidden shadow-2xl h-full">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-6 right-6 z-[130] w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 hover:bg-black/60 transition-all active:scale-90"
        >
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain pb-24">
          <div className="w-full">
            <Image src={"/contest/11.1.png"} alt="" width={800} height={600} className="w-full h-auto" />
          </div>
