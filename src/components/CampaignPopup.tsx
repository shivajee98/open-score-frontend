...
export default function CampaignPopup() {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-6 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-700">
            <div
                className="relative w-full max-w-[420px] bg-white rounded-[2.5rem] overflow-hidden shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] animate-in zoom-in-95 slide-in-from-bottom-10 duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]"
            ...
            >
                ...
                <button
                    onClick={handleClose}
                    className="absolute top-6 right-6 z-50 w-11 h-11 bg-white/10 hover:bg-white/20 backdrop-blur-xl rounded-full flex items-center justify-center text-white border border-white/20 transition-all hover:scale-110 active:scale-90 shadow-lg"
                >
                ...
                <div className={cn(
                    ...
                    <h3 className="text-3xl font-black text-slate-950 leading-[1.1] mb-4 tracking-tight">
                        {campaign.title}
                    </h3>
                    ...
                    <div className="space-y-4">
                        <button
                            ...
                            className="w-full h-[72px] bg-slate-950 hover:bg-indigo-600 text-white rounded-[1.25rem] font-black uppercase tracking-[0.2em] text-sm flex items-center justify-center gap-4 transition-all hover:shadow-[0_20px_40px_-10px_rgba(79,70,229,0.3)] active:scale-[0.97] group"
                        >
                            <span>Explore Now</span>
                            <div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center group-hover:bg-white/20 transition-colors">
                                {campaign.link?.startsWith('http') ? (
