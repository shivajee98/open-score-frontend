...
import { Wallet, Smartphone, Landmark, ScanBarcode, Send, History, Zap, CreditCard, ShieldCheck, QrCode, Flame, Droplets, Wifi, LayoutGrid, Tv, TrendingUp, Lock, ChevronLeft, ChevronRight, Bell, Headphones, Eye, EyeOff, Gift } from 'lucide-react';
...
export default function CustomerHome() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setActiveBanner((prev: number) => (prev === banners.length - 1 ? 0 : prev + 1));
        }, 8000); // Slower speed
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-32">
            ...
            <div className={`px-4 pt-8 pb-10 relative overflow-hidden shadow-2xl ${isMerchant ? 'bg-linear-to-br from-emerald-950 via-green-900 to-teal-950' : 'bg-linear-to-br from-slate-900 via-indigo-950 to-violet-950'}`}>
                {/* Main Gradient Background Overlay */}
                <div className={`absolute inset-0 z-0 ${isMerchant ? 'bg-linear-to-br from-emerald-900/50 via-emerald-950/50 to-teal-900/50' : 'opacity-50'}`}></div>
                ...
                <div className="absolute inset-0 z-0 opacity-20">
                    <div className="absolute top-[15%] left-[5%] w-px h-40 bg-linear-to-b from-transparent via-cyan-400 to-transparent transform rotate-45 animate-pulse"></div>
                    <div className="absolute bottom-[20%] right-[10%] w-px h-32 bg-linear-to-b from-transparent via-cyan-400 to-transparent transform -rotate-12 animate-[pulse_4s_infinite]"></div>
                </div>
                ...
                <div className="relative group z-10 mx-auto max-w-sm">
                    ...
                    {/* Card Container */}
                    <div className={`relative ${isMerchant ? 'bg-emerald-500/40' : 'bg-white/5'} backdrop-blur-xl rounded-2xl py-1.5 px-3 flex items-center justify-between border-[1.5px] ${isMerchant ? 'border-emerald-300' : 'border-white/10'} shadow-[0_0_20px_rgba(var(--theme-glow),0.2),inset_0_0_20px_rgba(var(--theme-glow),0.1)] overflow-hidden`}>
                        ...
                        {[
                            ...
                            <div key={i} className="flex flex-col items-center gap-1 active:scale-95 transition-all cursor-pointer">
                                <Link to={item.href || '#'} className="contents">
                                    ...
                                    <span className="text-[8px] font-black text-slate-700 uppercase tracking-widest text-center">{item.label}</span>
                                </Link>
                            </div>
                        ))}
                    </div>
            ...
            <div className="px-4 mb-3">
                <div onClick={() => navigate('/customer/pay?scan=true')} className="cursor-pointer group">
                    <div className="bg-linear-to-r from-amber-200 via-yellow-400 to-amber-500 p-0.5 rounded-2xl shadow-xl shadow-amber-500/20 active:scale-[0.98] transition-transform">
                        <div className="bg-slate-900 rounded-[0.9rem] px-4 py-3 relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between relative z-10">
                                <div className="flex-1">
                                    <h3 className="text-transparent bg-clip-text bg-linear-to-r from-amber-200 via-yellow-200 to-amber-100 font-black text-sm leading-tight tracking-tight mb-1">
                                        Get Your Need Done
                                    </h3>
                                ...
                                <div className="w-8 h-8 rounded-xl bg-linear-to-br from-amber-300 to-yellow-600 flex items-center justify-center text-slate-900 shadow-lg group-hover:scale-110 transition-transform">
            ...
            {/* KYC Alert (If any) */}
            {
                !isMerchant && kycLoan && (
                    <div className="px-4 mb-8">
                        <Link to={`/customer/loan/status/${kycLoan.id}`}>
                ...
                isMerchant && !user.pincode && (
                    <div className="px-4 mb-6">
                        <div onClick={() => setShowClaimModal(true)} className="cursor-pointer">
                            <div className="bg-linear-to-r from-purple-600 to-indigo-600 p-3 rounded-2xl shadow-xl shadow-purple-900/30 border-4 border-white/20 flex items-center justify-between group active:scale-[0.98] transition-all overflow-hidden relative">
                                <div className="flex items-center gap-3 relative z-10">
            ...
            <div className="relative mt-4 z-30 mb-10 group px-1">
                {/* Left Arrow - Outside */}
                <button
                    onClick={() => setActiveBanner((prev: number) => (prev === 0 ? banners.length - 1 : prev - 1))}
                    ...
                    >
                        {banners.map((banner, i) => (
                            <div
                                ...
                                className={`w-[88%] h-24 mr-3 ${banner.color} rounded-2xl py-4 px-6 shrink-0 flex flex-col justify-center shadow-2xl shadow-slate-900/40 cursor-pointer border border-white/10 relative overflow-hidden transition-all duration-300 ${i === activeBanner ? 'scale-100 opacity-100' : 'scale-95 opacity-50'}`}
                            >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-3 mt-2">
                                            <div className="flex flex-col">
                                                ...
                                                <span className={`font-black tracking-tighter inline-block ${i === 0 ? 'text-lg bg-linear-to-r from-amber-200 via-yellow-400 to-amber-200 bg-clip-text text-transparent drop-shadow-[0_0_12px_rgba(251,191,36,0.4)]' : 'text-xs text-white'}`}>
                                                    {banner.amount}
                                                </span>
                                            </div>
                                            {i === 0 && (
                ...
                {/* Right Arrow - Outside */}
                <button
                    onClick={() => setActiveBanner((prev: number) => (prev === banners.length - 1 ? 0 : prev + 1))}
